...
export class AIService {
  ...
  async enhanceComment(
    ...
    switch (provider) {
      case "openai":
        return this.openaiProvider.enhanceComment(
          transcript,
          codeContext,
          languageId
        );
      case "anthropic":
        return this.anthropicProvider.enhanceComment(
          transcript,
          codeContext,
          languageId
        );
  ...
  async testConnection(): Promise<{ openai: boolean; anthropic: boolean }> {
    ...
    const openaiTest = config.get<string>("openai.apiKey")
      ? await this.openaiProvider.testConnection()
      : false;
    const anthropicTest = config.get<string>("anthropic.apiKey")
      ? await this.anthropicProvider.testConnection()
      : false;
    ...
    try {
      ...
      if (!apiKey) {
        return {
          ...
          text: transcript,
          error:
            "OpenAI API Key nicht konfiguriert. Verwende: Voice Doc: Configure AI APIs",
        };
      }
    ...
    try {
      ...
      if (!apiKey) {
        return {
          ...
          text: transcript,
          error:
            "Anthropic API Key nicht konfiguriert. Verwende: Voice Doc: Configure AI APIs",
        };
      }
